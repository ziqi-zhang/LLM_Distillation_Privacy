...
def evaluate_blimp_main(
    ...
    original_save_path = osp.join(loss_save_dir, f"original_{len(dataset['train'])}.pt")
    if not osp.exists(original_save_path):
        ...
        if dist.get_rank() == 0:
            torch.save((train_original, validation_original), original_save_path)
    else:
        train_original, validation_original = torch.load(original_save_path)
    ...
    for layer_idx in range(num_blocks):
        layer_result_path = osp.join(loss_save_dir, f"layer_{layer_idx}_{len(dataset['train'])}.pt")
        if not osp.exists(layer_result_path):
            training_res = torch.zeros((args.num_perturbation_models + 1, len(dataset["train"])))  
            validation_res = torch.zeros((args.num_perturbation_models + 1, len(dataset["test"])))
            original_save_path = osp.join(loss_save_dir, f"original_{len(dataset['train'])}.pt")
            assert osp.exists(original_save_path)
            train_original, validation_original = torch.load(original_save_path)
            ...
            for idx, seed in enumerate(perturbation_seeds):
                ...
                save_path = osp.join(loss_save_dir, f"layer_{layer_idx}_seed_{seed}_{len(dataset['train'])}.pt")
                if not osp.exists(save_path):
                    ...
                    if args.model_type == "gpt2":
                        model.transformer.h[layer_idx].load_state_dict(perturb_model.transformer.h[layer_idx].state_dict())
                    elif args.model_type == "opt":
                        model.model.decoder.layers[layer_idx].load_state_dict(perturb_model.model.decoder.layers[layer_idx].state_dict())
                    else:
                        raise NotImplementedError
                    model = wrap_model(args, model, ds_config)
                    
                    train_save_path = osp.join(loss_save_dir, f"layer_{layer_idx}_seed_{seed}_train.pt")
                    if os.path.exists(train_save_path):
                        train_loss = torch.load(train_save_path)
                    else:
                        train_loss = mia_fn_train(
                            model=model, 
                            desc=f"layer {layer_idx}/{num_blocks} model {idx+1}/{len(perturbation_seeds)} on training {loss_save_dir}"
                        )
                        if dist.get_rank() == 0:
                            torch.save(train_loss, train_save_path)
                    
                    valid_save_path = osp.join(loss_save_dir, f"layer_{layer_idx}_seed_{seed}_validation.pt")
                    if os.path.exists(valid_save_path):
                        validation_loss = torch.load(valid_save_path)
                    else:
                        validation_loss = mia_fn_val(
                            model=model, 
                            desc=f"layer {layer_idx}/{num_blocks} model {idx+1}/{len(perturbation_seeds)} on validation {loss_save_dir}"
                        )
                        if dist.get_rank() == 0:
            ...
            if dist.get_rank() == 0:
                torch.save((training_res, validation_res), layer_result_path)
        else:
            print_rank(f"Loading results from {layer_result_path}...")
            training_res, validation_res = torch.load(layer_result_path)
        if dist.get_rank() == 0:
            torch.save((training_res, validation_res), layer_result_path)
            compute_and_save_results(training_res, validation_res, loss_save_dir, f"{mia_name}_layer_{layer_idx}", args)
            train_metrics, val_metrics = get_statistics_mope(training_res, validation_res)
            layer_aucs.append(compute_AUC(train_metrics, val_metrics))
    if dist.get_rank() == 0:
        plot_layer_auc(layer_aucs, args.save, mia_name)
    return training_res, validation_res

def plot_layer_auc(layer_aucs, save_dir, mia_name):
    import matplotlib.pyplot as plt
    plt.plot(layer_aucs)
    plt.xlabel("Layer")
    plt.ylabel("AUC")
    plt.title(f"{mia_name} AUC by layer")
    plt.savefig(osp.join(save_dir, f"{mia_name}_layer_auc.png"))
    plt.close()

    # Also save to a json file
    auc_dict = {f"layer_{i}": auc for i, auc in enumerate(layer_aucs)}
    with open(osp.join(save_dir, f"{mia_name}_layer_auc.json"), "w") as f:
        json.dump(auc_dict, f, indent=4)
        
def check_all_mia_results(args):
    for mia_name in ["loss"]:
        save_path = os.path.join(args.save, f"{mia_name}_layer_auc.json")
...
def evaluate_all_mi(args, tokenizer, dataset, ds_config, device):
    ...
    for mia_name, mia_fn_train, mia_fn_val in mia_name_fn_pairs:
        training_res, validation_res = evaluate_blimp_main(
            args, tokenizer, mia_fn_train, mia_fn_val, mia_name, dataset, ds_config, device
        )
...
def compute_and_save_results(training_res, validation_res, save_dir, mia_name, args):
    train_metrics, val_metrics = get_statistics_mope(training_res, validation_res)
    ...
    plot_ROC(
        ...
        save_name=os.path.join(save_dir, f"ROC_mope_{mia_name}.png")
    )
    plot_ROC(
        train_metrics, val_metrics, title="ROC", log_scale=True, show_plot=True, 
        save_name=os.path.join(save_dir, f"ROC_log_mope_{mia_name}.png")
    )
    results = {
        "AUC": AUC,
        "tpr_25": tpr_25,
    ...
    save_path = os.path.join(save_dir, f"mope_results_{mia_name}.json")
