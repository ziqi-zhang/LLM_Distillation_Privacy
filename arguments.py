...
def add_distillm_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--init-student", choices=["pretrain", "depth_prune", "width_prune",], default="pretrain")
    return parser

def add_mi_args(parser: argparse.ArgumentParser):
    group = parser.add_argument_group('mi', 'mi configurations')
    
    group.add_argument("--train-data-ratio", type=float, default=None)
    group.add_argument("--train-data-sample-seed", type=int, default=79)
    # private is the first half, public is the second half
    group.add_argument("--train-data-mode", type=str, choices=["public", "private", "all"], default="all")
    # group.add_argument("--model-block-prune", action="store_true")
    group.add_argument("--private-train-data-split", type=str, default=None)

    group.add_argument("--sigma", type=float, default=0.005)
    group.add_argument("--num-perturbation-models", type=int, default=5)

    
    # For multi-teacher, is a list of str, split by ","
    group.add_argument("--teacher-multi-model-paths", type=str, default=None)

    group.add_argument("--same_domain_calibration_path", type=str, default=None)
...
def add_feature_distill_args(parser: argparse.ArgumentParser):
    group = parser.add_argument_group('feature_distill', 'feature distill configurations')
    
    group.add_argument("--distill_block_output", action="store_true")
    group.add_argument("--distill_attention_weight", action="store_true")
    group.add_argument("--distill_attention_output", action="store_true")
    return parser
...
def get_args():
    ...
    parser = add_feature_distill_args(parser)
    parser = add_mi_args(parser)
    ...
    if args.type == "eval_main":
        ...
        args.save = save_path
    elif args.type == "lm" or args.type == "feature_kd":
        ...
        # save_path = os.path.join(
        #     args.save,
        #     (f"{args.ckpt_name}" + f"-{args.peft_name}" if args.peft_name is not None else ""),
        #     (f"e{args.epochs}-bs{args.batch_size}-lr{args.lr}") + \
        #     (f"-kd{args.kd_ratio}" if args.kd_ratio is not None else "") + \
        ...
        #     (f"e{args.epochs}-bs{args.batch_size}-lr{args.lr}-kd{args.kd_ratio}") + \
        #     (f"-mp{args.model_parallel_size}" if args.model_parallel > 0 else "") + \
        #     (f"-lora-{args.peft_lora_r}-{args.peft_lora_alpha}-{args.peft_lora_dropout}" if args.peft == "lora" else "") + \
        #     args.save_additional_suffix
        # )
        ...
        #     (f"{args.ckpt_name}" + f"-{args.peft_name}" if args.peft_name is not None else "" + \
        #      f"-{args.teacher_ckpt_name}" + f"-{args.teacher_peft_name}" if args.teacher_peft_name is not None else ""),
        #     (f"e{args.epochs}-bs{args.batch_size}-lr{args.lr}-lm{args.lm_coef}" + \
        #         (f"-mp{args.model_parallel_size}" if args.model_parallel > 0 else "")) + \
        #     (f"-lora-{args.peft_lora_r}-{args.peft_lora_alpha}-{args.peft_lora_dropout}" if args.peft == "lora" else ""),
