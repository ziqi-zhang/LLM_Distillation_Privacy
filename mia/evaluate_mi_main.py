...
def run_model(args, tokenizer, model, dataset: PromptDataset, epoch, device, desc=None):
    ...
    with torch.no_grad():
        for it, (model_batch, no_model_batch) in enumerate(tqdm(dataloader, desc=desc, disable=(dist.get_rank() != 0))):
            ...
            #     print_rank("############### Response ###############")
            #     # print_rank(no_model_batch.keys())
            #     print_rank(tokenizer.decode(no_model_batch["rest_ids"][0], skip_special_tokens=True))
            #     print_rank("############### End ###############")
    ...
    return all_lm_losses.cpu(), mean_lm_loss
...
def evaluate_mi_main(args, tokenizer, model, dataset: PromptDataset, split, epoch, device):
    len_dataset = len(dataset)
    
    save_path = os.path.join(args.save, f"{split}_{len_dataset}_lm_losses.pt")
    
    if not os.path.exists(save_path):
        all_lm_losses, lm_loss = run_model(args, tokenizer, model, dataset, epoch, device)
        torch.save(all_lm_losses, save_path) 
    else:
        all_lm_losses = torch.load(save_path)
        lm_loss = all_lm_losses.mean().item()
