...
class LMTrainDataset(Dataset):
    def __init__(self, args, tokenizer, path, split, num, ratio, rng_sample: random.Random):
        ...
        if num == -1:
            ...
            self.num = num
        self.lm_ctx_idxs = list(range(self.num))

        print_rank(f"Num LM instances: {len(self.lm_ctx)}")

    def set_subset(self, idxs):
        assert len(idxs) <= len(self.lm_ctx) and len(idxs) > 0 and max(idxs) < len(self.lm_ctx)
        self.lm_ctx_idxs = idxs
        self.num = len(idxs)
    ...
    def _get_lm(self, index):
        lm_ctx_idx = self.lm_ctx_idxs[index]
        data = self.lm_ctx[lm_ctx_idx]
