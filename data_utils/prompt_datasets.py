...
class PromptDataset(Dataset):
    ...
    def set_subset(self, idxs):
        assert len(idxs) <= len(self.data_idxs) and len(idxs) > 0 and max(idxs) < len(self.data_idxs)
        self.data_idxs = idxs
        self.num = len(idxs)

    def set_num(self, num):
        if num > 0:
            real_num = min(num, self.num)
        else:
            real_num = self.num
        if num < self.num:
            # self.data_idxs = random.sample(self.data_idxs, real_num)
            self.data_idxs = self.data_idxs[:real_num]
        self.num = real_num
    ...
    def __getitem__(self, index: int):
        data = self.data[self.data_idxs[index]]
