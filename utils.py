...
def save_json_rank(log_dict: Dict, save_path, rank=0):
    if not dist.is_initialized() or dist.get_rank() == rank:
        if not os.path.exists(save_path):
            ...
            with open(save_path, "r") as f:
                raw_log = json.load(f)
            # raw_log.update(log_dict)
            raw_log.append(log_dict)
            with open(save_path, "w") as f:
...
def get_model(args, device):
    ...
    if args.model_parallel:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            args.model_path, config=config, device_map={"": device}, torch_dtype=dtype,
            trust_remote_code=True,
        )
