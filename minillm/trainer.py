...
from utils import print_rank, save_rank, get_rank, all_gather, save_parallel, save_json_rank
...
class PPOTrainer():
    ...
    def train(self):
        ...
        for training_epoch in range(self.args.epochs):
            # Record epoch time
            for ppo_epoch in range(self.n_updates_per_batch):
                for it, batch in enumerate(self.train_dataloader):
                    ...
                    def get_log(log_stats, one_step_time):
                        ...
                        prefix = "train | data_epochs {:2d}({:2d})/{:2d} | inner iter: {:3d}/{:3d} | ppo epoch: {:2d}/{:2d} | global iter: {:6d}/{:6d}".format(
                            self.sampler.epochs, training_epoch,
                            self.args.epochs,
                            it,
            ...
            #     self.evaluate()
            self.evaluate(training_epoch, elapsed_time)
        ...
        # self.save_evals(preds, results, response_texts, f"final")
        return results
    ...
    def evaluate(self, epoch, train_epoch_time=None):
        eval_results = {}
        ...
        # self.save_evals(preds, eval_results, response_texts, f"epoch")
        ...
        if dist.get_rank() == 0:
            ...
            save_rank(eval_log_str, os.path.join(self.args.save, "eval_log.txt"))
            log_json = {
                "epoch": epoch,
                "metrics": {
                ...
                "train_epoch_time": train_epoch_time if train_epoch_time is not None else -1
            }
            save_json_rank(log_json, os.path.join(self.args.save, "eval_log.json"))
    ...
    def evaluate_ppo(self):  # noqa: C901
        ...
        with torch.no_grad():
            for batch in tqdm(self.eval_dataloader, "Generation Evaluation", disable=(not dist.get_rank() == 0)):
    ...
    def evaluate_pt(self):
        ...
        all_lm_losses = []
        all_kd_losses = []
        for batch in tqdm(self.eval_lm_dataloader, desc="LM Evaluation", disable=(not dist.get_rank() == 0)):
    ...
    def save_final(self, directory: Optional[str] = None):
        """Creates a checkpoint of the optimizer, scheduler and model"""
        """Creates checkpoint of optimizer, scheduler and a model"""
        base_ckpt_path = directory or self.args.save
        ckpt_dir = os.path.join(base_ckpt_path, f"final")
        os.makedirs(ckpt_dir, exist_ok=True)
        if self.args.model_parallel:
            raise NotImplementedError
        else:
