import os, sys
import os.path as osp
import json
import numpy as np
import pandas as pd
import seaborn as sns
from matplotlib import pyplot as plt

from pdb import set_trace as st

model_name = "gpt2"
kd_names = ["kd", "seqkd", "gkd", "imitkd", "minillm", "distillm"]
mia_nams = ["loss", "mink", "minkpp", "ref", "ref_pretrain_teacher", "zlib"]
model_sizes = ["large", "medium", "base"]


kd_to_label = {
    "kd": "KD",
    "seqkd": "SeqKD",
    "gkd": "GKD",
    "imitkd": "ImitKD",
    "minillm": "MiniLLM",
    "distillm": "DistillM",
    "no_kd": "No KD"
}
size_to_title = {
    "large": "GPT-2 Large",
    "medium": "GPT-2 Medium",
    "base": "GPT-2 Base",
}

def load_rouge_results(path):
    with open(path, "r") as f:
        results = json.load(f)
    return results[-1]["metrics"]["rougeL"]

def load_gap_results(path):
    with open(path, "r") as f:
        results = json.load(f)
    return abs(results["train"] - results["valid"])

results, gap_results = {}, {}
for size in model_sizes:
    results[size], gap_results[size] = {}, {}
    for kd in kd_names:
        results[size][kd] = load_rouge_results(f"results_6k/{model_name}/{size}/{kd}/eval_log.json")
    
    results[size]["no_kd"] = load_rouge_results(f"results_6k/{model_name}/student_ref_sft/{size}/eval_log.json")
        

save_dir = f"results_6k/{model_name}/heatmap"
if not osp.exists(save_dir):
    os.makedirs(save_dir)
    
    
df = pd.DataFrame(results).T
# print(df)
plt.figure(figsize=(6,3))
sns.heatmap(
    df, annot=True, cmap=sns.cubehelix_palette(as_cmap=True),
    xticklabels=[kd_to_label[kd] for kd in kd_names+["no_kd"]],
    yticklabels=[size_to_title[size] for size in model_sizes],
)  # annot=True 用于显示数值，cmap 设置颜色风格
# Rotate the tick labels for better visibility
plt.yticks(rotation=0)
plt.xticks(rotation=45)
plt.title("RougeL")
plt.tight_layout()
plt.savefig(f"{save_dir}/{model_name}_{size}_rougel_heatmap.png")
plt.clf()

