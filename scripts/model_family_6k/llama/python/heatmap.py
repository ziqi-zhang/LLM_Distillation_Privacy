...
for size in model_sizes:
    for kd in kd_names:
        results[size][kd]["mope"] = load_auc_results(f"results_6k/{model_name}/{size}/{kd}/mope/sigma_0.01_models_10/mope_results_loss.json")
...
#     df, annot=True, cmap=sns.cubehelix_palette(as_cmap=True),
#     xticklabels=[mia_to_label[mia] for mia in mia_nams],
#     yticklabels=[baseline_to_label[baseline] for baseline in ["teacher", "large", "medium", "base"]],
#     # vmin=min_value, vmax=max_value,
# )  # annot=True 用于显示数值，cmap 设置颜色风格
# # Rotate the tick labels for better visibility
# plt.xticks(rotation=45)
# plt.title("GPT-2 Baselines")
# plt.tight_layout()
